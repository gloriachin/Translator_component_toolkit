...
def use_name_resolver_ci():
    """Run legacy Name Resolver integration expectations against CI."""
...
def test_nameres_batch_search():
    ...
    results = TCT.name_resolver.batch_lookup(
        queries, return_top_response=True, return_synonyms=True
    )
...
def test_nameres_search(example_search):
    ...
    results = TCT.name_resolver.lookup(
        example_search["query"], return_top_response=False, limit=10
    )
